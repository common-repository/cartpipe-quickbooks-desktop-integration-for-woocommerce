...
class CP_Meta_Boxes {
	...
	public function __construct() {
        ...
        add_action( 'woocommerce_admin_order_data_after_order_details', 'CP_QB_Order_Meta_Box::output'); 
		/**
		 ...
		 */
		add_action( 'cp_process_shop_order_meta', 'CP_QB_Order_Meta_Box::save', 10, 2 );

		// Save Product Meta Boxes
		add_action( 'cp_process_product_meta', 'CP_QB_Product_Meta_Box::save', 10, 2 );
	...
	public function add_meta_boxes() {
		// Products
		add_meta_box( 'qb-product-data', __( 'CartPipe', 'cartpipe' ), 'CP_QB_Product_Meta_Box::output', 'product', 'side' );
		//add_meta_box( 'qb-order-data', __( 'CartPipe', 'cartpipe' ), 'CP_QB_Order_Meta_Box::output', 'shop_order', 'side' );
	}
	...
	public function save_meta_boxes( $post_id, $post ) {
		...
		// Check the nonce
		if ( empty( $_POST['cp_meta_nonce'] ) || ! wp_verify_nonce( $_POST['cp_meta_nonce'], 'cp_meta_nonce' ) ) {
			return;
		}
		...
		if ( in_array( $post->post_type, array( 'product', 'shop_coupon', 'shop_order' ) ) ) {
			...
			do_action( 'cp_process_' . $post->post_type . '_meta', $post_id, $_POST );
		}
	}
